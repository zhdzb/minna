...
## Phase 7: 第一轮用户体感反馈优化 (User Feedback Iteration 1)
- [ ] 24. **输入体验修复与优化 (Wanakana Input)** (BUGFIX)
  - [ ] 24.1 修复当假名最后输入 `n` 或单辅音时离焦导致的内容变回英文的问题（如在切换题目/提交前全量强制转译）
  - [ ] 24.2 在输入框周边新增**罗马音输入速查表** (TIPS，如：小 `ゃ` 可以打 `xya` / `lya`)
- [ ] 25. **AI 题库 Prompt 专项调优** (PROMPT ENGINEERING)
  - [ ] 25.1 强制在含有**人名**的造句题中，通过 `vocab_hints` 把名字的假名读音作为提示给出
  - [ ] 25.2 彻底修复生成题数与指定数量不符的问题（增强约束 prompt 体系限制 & 防截断）
- [ ] 26. **交互操作与界面优化** (UI/UX)
  - [ ] 26.1 `TrainingEngine.vue` 做题时，在底部新增方便的【上一题】/【下一题】快捷切换按钮
- [ ] 27. **高阶进度逻辑迭代** (EXECUTION)
  - [ ] 27.1 当答题类型为“混合实战考核 (ALL)”且通过时，自动批复给该课**所有启用的专项类型**通关印记
  - [ ] 27.2 [决策确认点] 现阶段未记录难度等级，向用户确认难度等级是否需要物理存档区分？
